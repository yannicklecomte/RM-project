...
for pwm in pwm_list:
    ...
    plt.title(f'Campaign {campaign:.0f}: All step UP starting from $\\gamma$ = {step[0]:.1f} \%')
    plt.xlabel('Time [s]')
...
#%% PLOT ALL NORMALIZED RESPONSES

# Number of indexes over which the end of the signal will be averaged
# This value is used to normalize the step
filt_width = 10
...
for pwm in pwm_list:
    ...
    for i, step in enumerate(steps):
        ...
        Ns = len(time_list)
        # Amount of shift index for plotting purposes
        N_shift_plot = 6
        crop_min = step_idx - N_shift_plot
        crop_max = crop_min + 32
        time_crop = time_list[crop_min:crop_max] - time_list[crop_min] - N_shift_plot*Ts
        thrust_crop = thrust_list[crop_min:crop_max]
        rpm_crop = rpm_list[crop_min:crop_max]
    
        # ==== RPM - Normalization ====
        rpm_offset = rpm_crop[N_shift_plot]  
        rpm_final = np.mean(rpm_crop[-filt_width:])  
        rpm_norm = (rpm_crop - rpm_offset) / (rpm_final - rpm_offset)
        target_pwm_map = ((step[1] - 5) / 5) * 100 
        
        # ==== THRUST - Normalization ====
        thrust_offset = thrust_crop[N_shift_plot]  
        thrust_final = np.mean(thrust_crop[-filt_width:])  
        thrust_norm = (thrust_crop - thrust_offset) / (thrust_final - thrust_offset)
    ...
    ax.text(-0.17, 0.85, '$\\gamma_{cmd}(t)$', fontsize=14, color='red',
    ...
    ax.set_xlim(-0.2, 0.8); ax.set_ylim(-0.1, 1.1); ax.set_yticks([0, 0.5, 1])
    fig_name = PLT_PATH + os.sep + f'ALL_norm_step_UP_{ref_pwm_map:.1f}_refPWM.pdf'
    plt.savefig(fig_name, format='pdf', dpi=400, bbox_inches='tight')
    plt.show()


#%% FITTING ALL THE STEPS
...
for pwm in pwm_list:
    ...
    for i, step in enumerate(steps):
        ...
        # ==== Normalize RPM: from 0 to 1 ====
        rpm_offset = rpm_crop[0]
        rpm_final = np.mean(rpm_crop[-filt_width:])
        rpm_norm = (rpm_crop - rpm_offset) / (rpm_final - rpm_offset)

        # ==== Normalize Thrust: from 0 to 1 ====
        thrust_offset = thrust_crop[0]
        thrust_final = np.mean(thrust_crop[-filt_width:])
        thrust_norm = (thrust_crop - thrust_offset) / (thrust_final - thrust_offset)
        ...
        rpm_pred = odeint(first_order, thrust_norm[0], time_crop, args=(u, tau_pred))[:, 0]
        data_dict[pwm][step]["tau"] = tau_pred

        # Plot comparison
        plt.figure(figsize=(5, 4))
        plt.plot(time_crop, rpm_norm, label="Measured")
        plt.plot(time_crop, rpm_pred, label=f"Model $\\tau$={tau_pred:.2f}s")
        plt.legend()
        plt.xlabel("Time [s]")
