...
data_path = os.path.join("Model_Opti_Controller_Sequences", f'Kp_{Kp}_Kd_{Kd}_Ne_{Ne}', 'data')
...
#%% Generate the time serie sequence from the model ===========================

# At the best controller defined in the previous block
gamma_clip = 8
theta_model, _, _, _, _, time_list_model, _, _ = sys.controller_simualtion([Kp, Kd], target_list, dt, gamma_clip)


#%% PLots the data ============================================================
     
# Define the saving paht for the plots
plt_path = os.path.join("Model_Opti_Controller_Sequences", f'Kp_{Kp}_Kd_{Kd}_Ne_{Ne}', 'plots')
os.makedirs(plt_path, exist_ok=True)

# -------------------------------------------------------------------------
# PLOT 1: Current time serie of balance alone
# -------------------------------------------------------------------------
...
plt.grid(True)
plt.tight_layout()
filename = os.path.join(plt_path, 'mean_time_serie.png')
...
filename = os.path.join(plt_path, 'cost_over_iter.png')
plt.savefig(filename, dpi=300, bbox_inches='tight')
plt.show()

# -------------------------------------------------------------------------
# PLOT 3: Plot times series of balance vs simu for a given controller
# -------------------------------------------------------------------------
...
plt.plot(time_list, ensemble_avg, color='blue', label='Controlled balance', zorder=3)
plt.plot(time_list_model, theta_model, c='green',  linestyle='-.', label='Digital twin prediction', zorder=2)
...
plt.grid(True, linestyle='--', linewidth=0.7, alpha=0.8, zorder=1)
plt.xlim(0, 25)
filename = os.path.join(plt_path, 'model_vs_balance_best_controller.pdf')
