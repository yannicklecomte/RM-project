...
budget = Ns_init*Ne_init + Ns_iter*Ne_iter

xi = 0.01
campaign = f"Ns_init_{Ns_init}_Ne_init_{Ne_init}_Ns_iter_{Ns_iter}_Ne_iter_{Ne_iter}_xi_{xi}_Budget_{budget}"
...
random_idx_hf = np.random.choice(Nt_hf, size=Ns_init, replace=False)
...
for iter_ in range(Ns_iter):
    ...
    mu, std = gp_predict(X_grid, X_H, alpha, L, params_opt)
    ...
    for ax, d, title, col_map in zip(axs, data, titles, color_maps):
        ...
        ax.scatter(X_H[:Ns_init, 0], X_H[:Ns_init, 1], c='green', edgecolor='k', s=50, zorder=10, clip_on=False, label='High-Fidelity Training')
        ax.scatter(X_H[Ns_init:, 0], X_H[Ns_init:, 1], c='blue', edgecolor='k', s=50, zorder=10, clip_on=False, label='High-Fidelity Explored')
    ...
    #plt.tight_layout()
    fig.suptitle("$\\textit{Case 2}$", fontsize=20, y = 1)
