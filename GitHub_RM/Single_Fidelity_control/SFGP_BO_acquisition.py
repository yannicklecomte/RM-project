...
Ne = 5
# Method to compute the cost function
cost_type = 'classical'
...
Data = "PD_SEQUENCE"
campaign = f"Ns_hf_{Ns_train_hf}_Ne_{Ne}_xi_{xi}_niter_{n_iter}"
...
params0 = np.log([1.0, 1.0, 0.1])
res = minimize(fct.log_marginal_likelihood, params0, args=(X_H, y_H, MLE_history, param_history), method='L-BFGS-B')
...
"Other notes: - Training high fig set -> 1 epsiode\n"
...
if __name__ == '__main__':   # Program entrance
    ...
    try:
        ...
        pL, pR = fct.setup()
        convergence_history = np.zeros((n_iter))
        ...
        for i in range(n_iter):
            ...
            if perform_hpo:
                MLE_history = []
                param_history = []
                res = minimize(fct.log_marginal_likelihood, last_params, args=(X_H, y_H, MLE_history, param_history), method='L-BFGS-B')
            ...
            # -----------------------------------------------------------------
            x_next, *_ = fct.propose_location_hybrid(X_H, y_H, opt_params, bounds, xi, grid_resolution=20, n_restarts=5)
                
            # Store the current best value for the convergence history
            convergence_history[i] = np.min(y_H)
                
            # -----------------------------------------------------------------
            # STEP 3: Plot and save the current prediction and the nxt sampling
            # -----------------------------------------------------------------

            fct.plot_n_save_BO(X_H, y_H, opt_params, Ns_train_hf, x_next, bounds, PLT_PATH, DATA_PATH, xi, iteration=i)
            # -----------------------------------------------------------------
            ...
            plt.plot(np.arange(n_iter), convergence_history, '--bo')
            plt.grid()
